...
describe("skills ownership", () => {
  ...
  it("records destination publisher source on transfer-created official badges", async () => {
    const patch = vi.fn(async () => {});
    const insert = vi.fn(async () => "auditLogs:1");
    const deleteRow = vi.fn(async () => {});
    const skill = {
          ...
          get: vi.fn(async (id: string) => {
            ...
            if (id === "publishers:org") {
              return {
                ...
                handle: "team",
                displayName: "Team",
                official: { byUserId: "users:admin", at: 222 },
              };
            }
            return null;
          }),
  ...
  it("replaces stale publisher official badges when moving into an official org", async () => {
    const patch = vi.fn(async () => {});
    const insert = vi.fn(async () => "auditLogs:1");
    const deleteRow = vi.fn(async () => {});
    const staleBadge = {
    ...
    expect(patch).toHaveBeenCalledWith("skillBadges:official", destinationBadge);
    expect(patch).toHaveBeenCalledWith(
      "skills:source",
      expect.objectContaining({
        badges: { official: destinationBadge },
      }),
    );
    expect(patch).toHaveBeenCalledWith(
      "skillSearchDigest:source",
    ...
    expect(deleteRow).not.toHaveBeenCalledWith("skillBadges:official");
  });

  it("replaces stale badges from a currently official source publisher", async () => {
    const patch = vi.fn(async () => {});
    const insert = vi.fn(async () => "auditLogs:1");
    const deleteRow = vi.fn(async () => {});
    const staleBadge = {
    ...
    const skill = {
      _id: "skills:source",
      slug: "portable-current-source",
      displayName: "Portable Current Source",
      ownerUserId: "users:actor",
      ownerPublisherId: "publishers:personal",
      badges: { official: staleBadge },
      softDeletedAt: undefined,
          ...
          get: vi.fn(async (id: string) => {
            ...
            if (id === "publishers:personal") {
              return {
                ...
                handle: "actor",
                linkedUserId: "users:actor",
                official: { byUserId: "users:admin", at: 333 },
              };
            }
            if (id === "publishers:org") {
              return {
            ...
            if (table === "skills") {
              return {
                withIndex: (name: string, build: (q: ReturnType<typeof chainEq>) => unknown) => {
                  ...
                  return {
                    unique: async () =>
                      constraints.slug === "portable-current-source" ? skill : null,
                  };
                },
              };
            }
  ...
  it("preserves manual official badges when moving between community publishers", async () => {
    const patch = vi.fn(async (_id: string, _value: Record<string, unknown>) => {});
    const insert = vi.fn(async () => "auditLogs:1");
    const deleteRow = vi.fn(async () => {});
    const manualBadge = { byUserId: "users:admin", at: 111 };
    const skill = {
      _id: "skills:source",
    ...
    await expect(
              ...
              if (table === "skills") {
                return {
                  withIndex: (name: string, build: (q: ReturnType<typeof chainEq>) => unknown) => {
                    ...
                    if (name !== "by_slug") throw new Error(`unexpected skills index ${name}`);
                    return {
                      unique: async () => (constraints.slug === "manual-official" ? skill : null),
                    };
                  },
                };
              }
      ...
      ok: true,
      transferred: true,
      skillSlug: "manual-official",
    });

    expect(deleteRow).not.toHaveBeenCalledWith("skillBadges:official");
    const skillPatch = patch.mock.calls.find((call) => call[0] === "skills:source")?.[1];
    expect(skillPatch).toMatchObject({ ownerPublisherId: "publishers:org" });
    expect(Object.hasOwn(skillPatch ?? {}, "badges")).toBe(false);
    expect(patch).toHaveBeenCalledWith(
      "skillSearchDigest:source",
      expect.objectContaining({
        ownerPublisherId: "publishers:org",
